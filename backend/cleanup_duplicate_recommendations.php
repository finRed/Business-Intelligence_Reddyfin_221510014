...
if ($_SESSION['role'] !== 'admin' && $_SESSION['role'] !== 'hr') {
    die("Only admin or HR can perform this operation");
}

$db = new Database();

try {
    echo "<h2>🧹 Pembersihan Rekomendasi Duplikat</h2>\n";
    echo "<p>Mencari dan menghapus rekomendasi duplikat untuk karyawan yang sama...</p>\n";
    ...
    if ($result->num_rows === 0) {
        echo "<p>✅ Tidak ada rekomendasi duplikat ditemukan.</p>\n";
    } else {
        echo "<p>⚠️ Ditemukan " . $result->num_rows . " karyawan dengan rekomendasi duplikat:</p>\n";
        echo "<table border='1' style='border-collapse: collapse; width: 100%;'>\n";
        echo "<tr><th>Employee ID</th><th>Manager ID</th><th>Jumlah Duplikat</th><th>Recommendation IDs</th><th>Tanggal Dibuat</th><th>Action</th></tr>\n";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>\n";
            echo "<td>{$eid} ({$info['employee_name']})</td>\n";
            echo "<td>{$recommended_by} ({$info['manager_name']})</td>\n";
            echo "<td>{$duplicate_count}</td>\n";
            echo "<td>" . implode(', ', $recommendation_ids) . "</td>\n";
            echo "<td>" . implode('<br>', $created_dates) . "</td>\n";
            ...
            if (!empty($ids_to_delete)) {
                ...
                if ($db->query($delete_sql)) {
                    ...
                    echo "<td style='color: green;'>✅ Dihapus {$deleted_count} rekomendasi lama (kept ID: {$newest_id})</td>\n";
                } else {
        ...
        echo "</table>\n";
        echo "<p><strong>📊 Total rekomendasi duplikat yang dihapus: {$total_deleted}</strong></p>\n";
    }
    ...
    if ($remaining_result->num_rows === 0) {
        ...
        echo "<table border='1' style='border-collapse: collapse; width: 100%;'>\n";
        echo "<tr><th>ID</th><th>Karyawan</th><th>Manager</th><th>Type</th><th>Durasi</th><th>Tanggal</th></tr>\n";
        ...
        echo "</table>\n";
        echo "<p><strong>Total rekomendasi pending tersisa: " . $remaining_result->num_rows . "</strong></p>\n";
    }
...
echo "<p><a href='../frontend/public/index.html'>← Kembali ke Dashboard</a></p>\n";
?> 